...
现在我们试着用一下这个中心方程，以简单一维晶格能带为例，在$ k = \frac{\pi}2 $处，

$$ \boxed{\left( \frac{\hbar^2 (\frac{\pi}2)^2}{2m} - E \right) C(\frac{\pi}2) + \sum_{n} V_n C(\frac{\pi}2 - G_n) = 0} $$

只保留一阶近似，为了求解，保留$ G = \frac{2 \pi}2$项

$$ \boxed{\left( \frac{\hbar^2 (\frac{\pi}2)^2}{2m} - E \right) C(\frac{\pi}2) + \sum_{n} V_G C(- \frac{\pi}2) = 0} $$
...
其中：\( V(\vec{r}) = V_{\text{eff}}(\vec{r}) + V_{\text{ext}}(\vec{r}) \)  
